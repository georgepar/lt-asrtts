...
def plot_ic():
    ...
    EIT_data = pd.read_excel("EIT_data.xlsx", sheet_name="Data", engine="openpyxl")
    ...
    GMM_features = data_reduction_analysis(
        scaler.fit_transform(EIT_features), EIT_label
    )
    ...
    fig = go.Figure(
        data=go.Scatter(x=n_components, y=bics, mode="lines+markers", name="BIC")
    )
    fig.add_trace(go.Scatter(x=n_components, y=aics, mode="lines+markers", name="AIC"))
